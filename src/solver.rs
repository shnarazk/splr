...
fn analyze(
    ...
    loop {
        ...
        unsafe {
            ...
            for q in &(*c).lits[((p != NULL_LIT) as usize)..] {
                ...
                to_rescale |= v.bump_activity(state.var_inc);
                if 0 < lvl && !state.an_seen[vi] {
                    ...
                    // if !state.an_seen[vi] {
                    //     println!("- ignore {}; it was flagged", q.int());
                    // } else {
                    //     println!("- ignore {}; its level is {}", q.int(), lvl);
                    // }
                }
            }
            // set the index of the next literal to ti
            while !state.an_seen[asgs.trail[ti].vi()] {
                // println!("- skip {}; it isn't flagged", asgs.trail[ti].int());
                ti -= 1;
            }
            p = asgs.trail[ti];
            let next_vi = p.vi();
            ...
            // to_rescale |= vars[next_vi].bump_activity(state.var_inc);
            path_cnt -= 1;
            if path_cnt <= 0 {
                break;
            }
    ...
    learnt[0] = p.negate();
    // println!("- append {}, the result is {:?}", learnt[0].int(), vec2int(learnt));
    let ret = simplify_learnt(asgs, cdb, state, vars);
    for l in &state.new_learnt[..] {
        to_rescale |= vars[l.vi()].bump_activity(state.var_inc * 0.3);
    }
    // /*
    // glucose heuristics
    ...
    while let Some(l) = state.last_dl.pop() {
        ...
        if cdb.clause[vars[vi].reason as usize].rank < lbd {
            to_rescale |= vars[vi].bump_activity(state.var_inc * 0.2);
        }
        */
        // we rewarded already. So get some back.
        if cdb.clause[vars[vi].reason as usize].rank >= lbd {
            to_rescale |= vars[vi].bump_activity(state.var_inc * -0.2);
        }
    }
