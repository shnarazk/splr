...
fn main() {
    ...
    let ans_file: Option<PathBuf> = match config.output_filename.as_str() {
        "-" => None,
        "" => Some(PathBuf::from(&config.directory).join(PathBuf::from(format!(
            ".ans_{}",
            config.cnf_file.file_name().unwrap().to_str().unwrap()
        )))),
        _ => Some(PathBuf::from(&config.directory).join(PathBuf::from(&config.output_filename))),
    };
    let proof_file: PathBuf =
        PathBuf::from(&config.directory).join(PathBuf::from(&config.proof_filename));
    let mut s = Solver::build(&config).expect("failed to load");
    let res = s.solve();
    match res {
        Ok(Certificate::SAT(_)) => {
            save_result(&s, &res, &input, ans_file);
...
fn save_proof(s: &Solver, input: &str, output: &PathBuf) {
    ...
    println!(
        "The certification was dumped to {}.",
