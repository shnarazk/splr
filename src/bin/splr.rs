...
fn main() {
    let config = ConfigOption::from_args();
    println!("{:?}", &config);
    if config.cnf.exists() {
        let cnfname = config.cnf.to_str().unwrap().to_string();
            ...
            Ok(Certificate::SAT(v)) => {
                if let Ok(out) = File::create(&result) {
                    ...
                    if let Err(why) = (|| {
                        buf.write_all(
                            format!(
                                "c An assignment generated by splr-{} for SATISFIABLE {}\nc\n",
                                VERSION,
                                cnfname,
                            )
                            .as_bytes(),
            ...
            Ok(Certificate::UNSAT(_)) => {
                if let Ok(out) = File::create(&result) {
                    ...
                    if let Err(why) = (|| {
                        buf.write_all(
                            format!(
                                "c The empty solution generated by splr-{} for UNSATISFIABLE {}\nc\n",
                                VERSION,
                                cnfname,
                            )
                            .as_bytes(),
