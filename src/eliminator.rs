...
impl Solver {
    ...
    pub fn add_cross(&mut self, vec: &[Lit]) -> bool {
        ...
        let cid = self.cp[ClauseKind::Permanent as usize].new_clause(&vec, 0, false, false);
    ...
    pub fn merge(&mut self, cp: ClauseId, cq: ClauseId, v: VarId) -> Option<Vec<Lit>> {
        ...
        // println!("merge generated {:?} from {} and {} to eliminate {}", vec2int(vec), p, q, v);
        Some(vec)
    }
    /// 7. merge(2)
    ...
    pub fn gather_touched_clauses(&mut self) -> () {
        ...
        let mut _len = self.eliminator.clause_queue.len();
    ...
    pub fn backward_subsumption_check(&mut self) -> bool {
            ...
            || self.eliminator.bwdsub_assigns < self.am.trail.len()
        {
            ...
            unsafe {
                let c = if cid == self.eliminator.bwdsub_tmp_clause.index {
    ...
    pub fn eliminate_var(&mut self, v: VarId) -> bool {
        ...
        unsafe {
            let pos = &mut self.vars[v].pos_occurs as *mut Vec<ClauseId>;
            let neg = &mut self.vars[v].neg_occurs as *mut Vec<ClauseId>;
