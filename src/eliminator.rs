...
impl EliminatorIF for Eliminator {
    ...
    fn enqueue_clause(&mut self, cid: ClauseId, c: &mut Clause) {
        if !self.in_use || !self.active || c.holds(Flag::Enqueued) {
            return;
        }
        let len = c.lits.len();
    ...
    fn enqueue_var(&mut self, v: &mut Var) {
        if !self.in_use || !self.active || v.holds(Flag::Enqueued) {
            return;
        }
    ...
    fn eliminate(
            ...
            || !self.clause_queue.is_empty()
        {
            ...
            while let Some(vi) = self.var_queue.pop() {
                ...
                if v.holds(Flag::EliminatedVar) || v.assign != BOTTOM {
                    continue;
                }
                v.check_sve_at += 1;
            ...
            unsafe {
                ...
                if cid.is_lifted_lit() {
                    ...
                    for l in &(*c).lits {
                        ...
                        if !v.holds(Flag::EliminatedVar) && 0 < v.level && nsum < tmp {
                            best = l.vi();
                            tmp = nsum;
                        }
                    }
                }
                if best == 0 || vars[best].holds(Flag::EliminatedVar) {
                    continue;
                }
                for p in 0..2 {
                    let cs = if p == 0 {
...
fn try_subsume(
    ...
    match subsume(cdb, cid, did) {
        Some(NULL_LIT) => {
            if !cid.is_lifted_lit()
                && cdb.clause[cid].holds(Flag::LearntClause)
                && cdb.clause[did].holds(Flag::LearntClause)
            {
...
fn strengthen_clause(
    ...
    debug_assert!(!cdb.clause[cid].holds(Flag::DeadClause));
...
fn eliminate_var(
    ...
    v.pos_occurs
        .retain(|&c| !cdb.clause[c].holds(Flag::DeadClause));
    v.neg_occurs
        .retain(|&c| !cdb.clause[c].holds(Flag::DeadClause));
                ...
                if let Some(vec) = merge(cdb, *p, *n, vi) {
                    ...
                    match vec.len() {
                        ...
                        _ => {
                            ...
                            if cdb.clause[*p].holds(Flag::LearntClause)
                                && cdb.clause[*n].holds(Flag::LearntClause)
                            {
