...
impl Eliminator {
    ...
    fn backward_subsumption_check(
        ...
        while !self.clause_queue.is_empty() || self.bwdsub_assigns < asgs.len() {
            ...
            if best == 0 || vars[best].is(Flag::ELIMINATED) {
                continue;
            }
            unsafe {
                for cs in &[
...
fn try_subsume(
    ...
    match subsume(cdb, cid, did) {
        Some(NULL_LIT) => {
            ...
            // );
            cdb.detach(did);
            elim.remove_cid_occur(vars, did, &mut cdb[did]);
...
fn check_to_merge(
    ...
    'next_literal: for l in &qb.lits {
        ...
        if l.vi() != v {
            for j in &pb.lits {
                if vars[j.vi()].is(Flag::ELIMINATED) {
                    continue;
                }
                if j.vi() == l.vi() {
                    if j.negate() == *l {
...
fn strengthen(cdb: &mut ClauseDB, cid: ClauseId, p: Lit) -> bool {
    ...
    if lits[0] == p || lits[1] == p {
        ...
        debug_assert!(1 < p.negate());
        let len = lits.len();
        cdb.watcher[p.negate() as usize].detach_with(cid);
        cdb.watcher[q.negate() as usize].register(r, cid);
        if len == 2 {
            // update another bocker
            cdb.watcher[r.negate() as usize].update_blocker(cid, q);
        }
    } else {
        ...
        if lits.len() == 2 {
            ...
            let q = lits[0];
            let r = lits[1];
            cdb.watcher[q.negate() as usize].update_blocker(cid, r);
            cdb.watcher[r.negate() as usize].update_blocker(cid, q);
        }
    }
    false
...
trait VarOrderIF {
    ...
    fn clear(&mut self, vars: &mut VarDB);
    fn len(&self) -> usize;
    fn is_empty(&self) -> bool;
    fn select_var(&mut self, vars: &VarDB) -> Option<VarId>;
...
impl VarOrderIF for VarOccHeap {
    ...
    fn insert(&mut self, vars: &VarDB, vi: VarId, upward: bool) {
