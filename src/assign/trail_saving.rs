...
impl TrailSavingIF for AssignStack {
    fn save_trail(&mut self, to_lvl: DecisionLevel) {
        ...
        if 2 <= dl {
            ...
            let activity_threshold = self.var[self.trail[lim2].vi()].activity;
            for i in (lim..lim2).rev() {
                ...
                if activity_threshold <= self.var[vi].activity {
                    self.insert_heap(vi);
                }
            }
            free = lim2;
    ...
    fn reuse_saved_trail(&mut self, cdb: &impl ClauseDBIF) -> PropagationResult {
        let q = self.stage_scale.trailing_zeros()
            + (cdb.derefer(crate::cdb::property::Tf64::LiteralBlockEntanglement) as u32) / 2;
