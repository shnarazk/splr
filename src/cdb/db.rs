...
impl ClauseDBIF for ClauseDB {
    ...
    fn new_clause(
        ...
        *num_clause += 1;
        ...
        self.clause.insert(cr);
        if len2 {
            *num_bi_clause += 1;
            binary_link.add(l0, l1, cr);
        } else {
            watch_cache[!l0].insert_watch(cr, l1);
            watch_cache[!l1].insert_watch(cr, l0);
        }
        RefClause::Clause(cr)
    }
    ...
    fn remove_clause(&mut self, mut cr: ClauseRef) {
    ...
    fn remove_clause_sandbox(&mut self, mut cr: ClauseRef) {
        // assert_eq!(self.clause.iter().skip(1).filter(|c| !c.is_dead()).count(), self.num_clause);
        let c = &mut cr.get_mut();
        debug_assert!(!c.is_dead());
        debug_assert!(1 < c.lits.len());
    ...
    fn transform_by_elimination(&mut self, mut cr: ClauseRef, p: Lit) -> RefClause {
        //
        ...
        if new_lits.len() == 2 {
            ...
            binary_link.add(lits[0], lits[1], cr);
            *num_bi_clause += 1;
    ...
    fn transform_by_updating_watch(
        ...
        watch_cache[!c.lits[new]].insert_watch(cr, c.lits[other]);
    ...
    fn reduce(&mut self, asg: &mut impl AssignIF, setting: ReductionType) {
        ...
        *num_reduction += 1;
        ...
        for op in perm.iter().skip(keep) {
            self.remove_clause(*op.to());
        }
    }
    fn reset(&mut self) {
        debug_assert!(1 < self.clause.len());
        for cr in &mut self.clause.iter() {
            let c = cr.get();
            if c.is(FlagClause::LEARNT)
                && !c.is_dead()
    ...
    fn validate(&self, model: &[Option<bool>], strict: bool) -> Option<ClauseRef> {
        for (i, cr) in self.clause.iter().enumerate() {
            let c = cr.get();
            if c.is_dead() || (strict && c.is(FlagClause::LEARNT)) {
                continue;
            }
            match c.evaluate(model) {
                Some(false) => return Some(*cr),
                None if strict => return Some(*cr),
                _ => (),
            }
        }
        None
    ...
    fn dump_cnf(&self, asg: &impl AssignIF, fname: &Path) {
        ...
        let nc = self.iter().filter(|cr| !cr.get().is_dead()).count();
